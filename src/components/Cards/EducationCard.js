...
function Card(props) {
    ...
    function nav(){
        setIsClicked(true);
        navigate("/Education/EditEducation", {replace:true});
    }
    ...
    return(
        ...
        <div className="card-body">
          ...
          <h6 className="card-subtitle mb-2 text-muted">{props.info.school}</h6>
          <p className="card-text">{props.info.country}</p>
          <div className="date">
              <p className="text-muted" >{props.info.start_date}     -      {props.info.end_date}</p></div>
        </div>
        {isClicked && <Routes>
                <Route path="EditEducation" element={<EditEducation info={props.info} />}></Route>
            </Routes>}
      </div>
    );
}
